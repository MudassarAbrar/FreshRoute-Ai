/**
 * Crop Reference Data — Static domain constants.
 *
 * These are agricultural domain knowledge values (perishability rates,
 * crop aliases, volatility indices) that don't change per-transaction.
 *
 * Dynamic marketplace data (buyers, transporters, prices) is queried
 * from the database via src/lib/marketplaceService.ts.
 */

export const MAUND_KG = 37.32

/** City-to-city road distances in km (reference only — OSRM used in production) */
...
/** Crop alias → canonical name mapping */
export const CROP_ALIASES: Record<string, string> = {
  tomato: "Tomato", tomatoes: "Tomato", tamatar: "Tomato", "ٹماٹر": "Tomato",
  potato: "Potato", potatoes: "Potato", aloo: "Potato", "آلو": "Potato",
  onion: "Onion", onions: "Onion", pyaaz: "Onion", "پیاز": "Onion",
  mango: "Mango", mangoes: "Mango", aam: "Mango", "آم": "Mango",
  kinnow: "Kinnow",
  banana: "Banana", bananas: "Banana",
  chili: "Green Chili", chilli: "Green Chili", "green chili": "Green Chili",
  "green chilli": "Green Chili", mirch: "Green Chili",
  okra: "Okra", bhindi: "Okra", "بھنڈی": "Okra",
  leafy: "Leafy Vegetables", spinach: "Leafy Vegetables", palak: "Leafy Vegetables",
}
...
export const CROP_VOLATILITY: Record<string, number> = {
  ...
  Kinnow: 0.4, Banana: 1.1, "Green Chili": 0.9, Okra: 0.95,
...
/** Perishability profiles per crop — base for spoilage engine */
...
/** Transport mode multipliers for spoilage engine */
...
export const MATCH_WEIGHTS = {
  priceFit: 0.3, quantityFit: 0.2, proximity: 0.2,
  reliability: 0.15, urgency: 0.15,
}

/** Provider (transporter/storage) scoring weights */
export const PROVIDER_MATCH_WEIGHTS = {
  cost: 0.25, proximity: 0.2, rating: 0.2,
  capabilityMatch: 0.2, spoilageRisk: 0.15,
}

/** Static weather reference (production should use a real weather API) */
export const WEATHER = {
  Multan: { tempC: 38, condition: "Clear · hot afternoon" },
  Lahore: { tempC: 34, condition: "Partly cloudy" },
}

/** Default crop prices — used only as fallback when DB has no price data */
